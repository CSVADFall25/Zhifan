...
function setup() {
    ...
    myPicker.position(0.95 * width, 120);


    //clear canvas button
    clearBtn = createButton('CLEAR CANVAS');
    clearBtn.position(20, 80);
...
function draw() {
    ...
    textSize(16);
    text("Drag a plant to imprint on canvas | Click and press =/- to rotate | Press 'delete' to remove a plant", 20, 135);
    textAlign(RIGHT, CENTER);
    text("Change canvas color", 0.95 * width - 20, 135);
...
function mousePressed() {
    ...
    //delete function: if delete/backspace is held and you click on a plant, remove it immediately
    ...
    if (deleteHeld) {
        ...
        // if delete is held but no plant under the cursor, bail out
        return;
    }


    // Selection (normal click behavior)
    for (let i = settledPlant.length - 1; i >= 0; i--) {
        let s = settledPlant[i];
...
class Flower {
    ...
    makePieceConfig(r) {
        ...
        return {
            //r = 花朵整体大小
            petalnumber: petalnumber,       // 花瓣数量
            tipScale: random(0.5, 0.8),
            r0: r * random(0.1, 0.2),   // 花蕊半径
            r1: r * random(0.4, 0.6),   // 底部控制点半径
            a1: -PI * random(0.6, 1.0) / petalnumber, // 左侧控制角（右侧镜像）
            r2: r * random(0, 0.5),   // 顶部控制半径
            a2: PI * random(0.1, 0.5)              // 顶部控制角（朝上）
        };
    }
    ...
    drwRdmFlw() {
        ...
        // 花蕊 (centered at 0,0 in local coordinates)
    ...
    // alias so Flower can be used in places that expect Maple-like API
    drwRdmMaple() {
        this.drwRdmFlw();
    }
    ...
    hitTest(mx, my) {
        // The leaf is drawn centered at this.pos — use a circular hit test.
        // Adjust multiplier for larger/smaller clickable area.
...
class Maple {
    ...
    // 只随机一次，返回一份可复用的叶瓣形状配置
    makePieceConfig(r) {
        return {
            tipScale: random(0.6, 0.8),       // 叶尖高度比例
            r1: r * random(0.4, 0.57),   // 底部控制半径
            a1: -PI * random(0.55, 0.8), // 左侧控制角（右侧镜像）
            r2: r * random(0.5, 0.7),   // 顶部控制半径
            a2: PI * 0.5              // 顶部控制角（朝上）
        };
    }
    ...
    drwRdmMaple() {
        ...
        const tipY = -this.r * this.cfgMain.tipScale;
        const cTop = color(this.colors[0]);     // 叶尖端颜色
        const cBot = color(this.colors[1]);     // 叶柄端颜色
        const cMid = lerpColor(cTop, cBot, 0.5);


        const gd = ctx.createLinearGradient(0, 0, 0, tipY);
        gd.addColorStop(0.00, cBot.toString()); // 叶柄端
        gd.addColorStop(0.50, cMid.toString()); // 中间色
        gd.addColorStop(1.00, cTop.toString()); // 叶尖端
        ctx.fillStyle = gd;
        ...
        const ctrlY = tipY + this.cfgMain.r2;           // y2（因 a2 = PI/2）
